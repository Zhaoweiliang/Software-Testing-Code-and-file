...
pix_val = img_gray.load()
print(np.shape(img_gray))
row = np.shape(img_gray)[0]
col = np.shape(img_gray)[1]
mat_im = np.zeros([col,row])
prob_im = np.zeros([col,row])

for i in range(col):
    for j in range(row):
        mat_im[i,j] = pix_val[i,j]
print(row,col)
for i in range(col):
    for j in range(row):
        if mat_im[i,j] <= Water_RGB[1] and mat_im[i,j] >= Water_RGB[0]:
            prob_im[i,j] = Water_prob # waterbodies
        elif mat_im[i,j] <= House_RGB[1] and mat_im[i,j] >= House_RGB[0]:
            prob_im[i,j] = Wood_House_Prob # wood and houses
        else:
            prob_im[i,j] = Road_Prob # roads
...
Burning = (a==2)
Burning_loc = locs_where(Burning)
print(FirePlace1.place[153,125]==2.0)
print(FirePlace1.place[200,200]==2.0)
print(FirePlace1.place[200,200]==2.0)
print(len(Burning_loc))
